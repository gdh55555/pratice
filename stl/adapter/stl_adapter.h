...
template <class Operation>
class biner2nd : public unary_function<typename Operation::first_argument_type, typename Operation::result_type>
{
    protected:
        Operation op;
        typename Operation::second_argument_type value;
    public:
        biner2nd(const Operation& x, const typename Operation::second_argument_type& y)
            : op(x), value(y) {}
...
template <class Operation, class T>
inline biner2nd<Operation> biner2nd(const Operation& op, const T& y)
{
    typedef typename Operation::second_argument_type    arg2_type ;
    return biner2nd<Operation>(op, arg2_type(x));
}
...
class pointer_to_unary_function : public binary_function<Arg1, Arg2, Result>
{
    protected:
...
template <class Arg1, class Arg2, class Result>
inline pointer_to_unary_function<Arg1, Arg2, Result> ptr_fun(Result (*x)(Arg1, Arg2))
{
    return pointer_to_unary_function<Arg1, Arg2, Result>(x);
}

template <class S, class T>
class mem_fun_t : public unary_funtion<T*, S>{
    public:
...
template <class S, class T>
class const_mem_fun_t : public unary_funtion<T*, S>{
    public:
...
template <class S, class T>
class mem_fun_ref_t : public unary_funtion<T, S>{
    public:
        explicit mem_fun_t(S (T::*pf)()) : f(pf) {}
        S operator()(T& r) const { return (p.*f)();}
    private:
        S (T::*f)();
...
template <class S, class T>
class const_mem_fun_ref_t : public unary_funtion<T, S>{
    public:
        explicit mem_fun_t(S (T::*pf)()const) : f(pf) {}
        S operator()(T& r) const { return (p.*f)();}
    private:
